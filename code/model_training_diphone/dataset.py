...
class BrainToTextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # --- PHASE 2: DIPHONE MODELING MODIFICATION ---
        # Convert phoneme sequences to diphone sequences
        # Model predicts 40*40+1=1601 classes (diphones + CTC blank)
        # We map 40 non-blank base units (39 phonemes + <sil>, indices 1-40)
        # to a 2D diphone space, then shift for CTC blank at index 0

        NUM_BASE_UNITS = 40  # 39 phonemes + 1 silence token
        diphone_sequences = []
        diphone_seq_lens = []

        for seq, seq_len in zip(batch['seq_class_ids'], batch['phone_seq_lens']):
            # Extract actual sequence (excluding padding)
            phoneme_seq = seq[:seq_len].tolist()

            # Convert phoneme indices (1-40) to base indices (0-39)
            base_indices = [idx - 1 for idx in phoneme_seq if idx != 0]

            # Pad with silence token index (39) at start and end
            silence_base_idx = NUM_BASE_UNITS - 1  # Index 39
            padded_base_indices = [silence_base_idx] + base_indices + [silence_base_idx]

            # Create diphone sequence
            diphone_seq = []
            for i in range(len(padded_base_indices) - 1):
                prev_idx = padded_base_indices[i]
                curr_idx = padded_base_indices[i + 1]

                # Diphone index = (prev * 40) + curr + 1 (shift for CTC blank at 0)
                diphone_idx = (prev_idx * NUM_BASE_UNITS) + curr_idx + 1
            ...
            diphone_sequences.append(torch.tensor(diphone_seq))
            diphone_seq_lens.append(len(diphone_seq))

        # Pad diphone sequences and replace original phoneme sequences
        batch['seq_class_ids'] = pad_sequence(diphone_sequences, batch_first=True, padding_value=0)
        batch['phone_seq_lens'] = torch.tensor(diphone_seq_lens)
