...
# Configure GPU device for model inference
...
with tqdm(total=total_test_trials, desc='Predicting phoneme sequences', unit='trial') as pbar:
    for session, data in test_data.items():
        ...
        for trial in range(len(data['neural_features'])):
            ...
            # Run model forward pass for decoding
...
# if using the validation set, let's calculate the aggregate word error rate (WER)
if eval_type == 'val':
    total_true_length = 0
