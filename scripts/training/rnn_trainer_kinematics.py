...
class BrainToTextDecoder_Trainer:
    ...
    def __init__(self, args):
        ...
        self.ctc_loss = torch.nn.CTCLoss(blank = 0, reduction = 'none', zero_infinity = True)  # zero_infinity=True for stability
    ...
    def compute_multitask_loss(self, outputs_dict, batch, input_lengths):
        """
        GATED Multi-Task Loss: Dynamically gate auxiliary loss based on confidence.

        Strategy: If the aux predictions have high uncertainty (low confidence),
        this indicates the articulatory features are unreliable for this batch
        (possibly silent speech). We reduce/zero the aux weight.

        Gating Mechanism:
        - Compute mean max-probability (confidence) for aux heads
        - If confidence < threshold, gate OFF aux loss
        - This is a self-supervised "silent speech detector"
        """
        # Base Weights
        AUX_WEIGHT_BASE = 0.2
        CONFIDENCE_THRESHOLD = 0.5  # Aux predictions must be this confident to be used

        target_lengths = batch['phone_seq_lens']

        # =================================================================
        # 1. Main Task (Diphone) - Always computed
        # =================================================================
        ...
        # =================================================================
        # 2. Aux Tasks with Confidence Gating
        # =================================================================
        loss_aux = 0.0
        aux_confidences = []
        ...
        if 'place' in outputs_dict and 'place_targets' in batch:
            place_probs = outputs_dict['place'].softmax(2)
            place_confidence = place_probs.max(dim=-1)[0].mean()  # Mean max prob
            aux_confidences.append(place_confidence)
        ...
        if 'manner' in outputs_dict and 'manner_targets' in batch:
            manner_probs = outputs_dict['manner'].softmax(2)
            manner_confidence = manner_probs.max(dim=-1)[0].mean()
            aux_confidences.append(manner_confidence)
        ...
        if 'voice' in outputs_dict and 'voice_targets' in batch:
            voice_probs = outputs_dict['voice'].softmax(2)
            voice_confidence = voice_probs.max(dim=-1)[0].mean()
            aux_confidences.append(voice_confidence)
        ...
        # =================================================================
        # 3. GATING: Compute effective aux weight based on confidence
        # =================================================================
        if len(aux_confidences) > 0:
            mean_aux_confidence = torch.stack(aux_confidences).mean()

            # Soft gating: Linear scale from 0 at threshold to 1 at 1.0 confidence
            # gate_factor = max(0, (confidence - threshold) / (1 - threshold))
            gate_factor = torch.clamp(
                (mean_aux_confidence - CONFIDENCE_THRESHOLD) / (1.0 - CONFIDENCE_THRESHOLD + 1e-6),
                min=0.0,
                max=1.0
            )

            effective_aux_weight = AUX_WEIGHT_BASE * gate_factor
        else:
            effective_aux_weight = 0.0
            gate_factor = torch.tensor(0.0)

        # =================================================================
        # 4. Total Loss
        # =================================================================
        total_loss = loss_main + (effective_aux_weight * loss_aux)
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # Skip backward pass if loss is NaN/Inf to prevent model corruption
            if torch.isnan(loss) or torch.isinf(loss):
                self.logger.warning(f'Batch {i}: Skipping update due to NaN/Inf loss')
                self.optimizer.zero_grad()
                ...
                if self.args['grad_norm_clip_value'] > 0:
                    grad_norm = torch.nn.utils.clip_grad_norm_(self.model.parameters(),
                                                   ...
                                                   error_if_nonfinite = False,  # Allow recovery from gradient spikes
                                                   foreach = True
                                                   )

                # Skip optimizer step if gradients are NaN/Inf
                if torch.isnan(grad_norm) or torch.isinf(grad_norm):
                    self.logger.warning(f'Batch {i}: Skipping update due to NaN/Inf gradients')
                    self.optimizer.zero_grad()
                else:
                    self.optimizer.step()
