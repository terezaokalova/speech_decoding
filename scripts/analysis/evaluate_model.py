...
# load csv file (optional - only used for corpus name lookup)
try:
    b2txt_csv_df = pd.read_csv(args.csv_path)
except FileNotFoundError:
    print(f'Warning: CSV file {args.csv_path} not found. Corpus names will be set to "unknown".')
    b2txt_csv_df = None
...
with tqdm(total=total_test_trials, desc='Predicting phoneme sequences', unit='trial') as pbar:
    for session, data in test_data.items():
        ...
        for trial in range(len(data['neural_features'])):
            ...
            # Convert 1601 diphone logits -> 41 phoneme logits for LM compatibility
            logits_tensor = torch.tensor(logits, device=device, dtype=torch.float32)
            logits = convert_diphone_logits_to_phoneme_logits(logits_tensor).cpu().numpy()
